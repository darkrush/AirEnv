...
def parse_senario(senario_file):
    ...
    fence_list = {}
    if 'fence_groups' in raw_senario_dict.keys():
        for key,group in raw_senario_dict['fence_groups'].items():
            number = group['num']
            group_list = []
            for _ in range(number):
                group_list.append({})
            ...
            fence_list[key]=group_list
        senario_dict['fence_groups'] = fence_list
    return senario_dict